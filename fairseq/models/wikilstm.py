...
@register_model('wikilstm')
class WikiLSTMModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        encoder = WikiLSTMEncoder(
            dictionary=task.source_dictionary,
            word_embed=word_embed,
            field_embed=field_embed,
            pos_embed=pos_embed,
            rpos_embed=rpos_embed,
            hidden_size=args.encoder_hidden_size,
            num_layers=args.encoder_layers,
            dropout_in=args.encoder_dropout_in,
            dropout_out=args.encoder_dropout_out,
            bidirectional=args.encoder_bidirectional,
            input_num_embeddings=input_num_embeddings
        )

        decoder = WikiLSTMDecoder(
...
# 自己写dataset的时候已经是right_pad的了，所以不用搞left_pad
class WikiLSTMEncoder(FairseqEncoder):
    """LSTM encoder."""
    def __init__(
        self, dictionary, word_embed, field_embed, pos_embed, rpos_embed,
        ...
        self.lstm = LSTM(
            input_size=input_num_embeddings,
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        bsz, seqlen, _  = src_tokens.size()   # batch * seqlen * 4
        ...
        x = torch.cat((word,field,pos,rpos),dim=2) # batch * seqlen * input_num_embeddings
        ...
        encoder_padding_mask = src_tokens[:,:,0].eq(self.padding_idx).t()
        

        # x: seqlen * batch * output_units
        # final_hiddens/final_cells: -1 * batch * output_units
        return {
...
class WikiAttentionLayer(nn.Module):
...
class WikiLSTMDecoder(FairseqIncrementalDecoder):
    """LSTM decoder."""
    def __init__(
        self, dictionary, embed_dim, word_embed, hidden_size=512, out_embed_dim=512,
        ...
        if attention:
            ...
            self.attention = WikiAttentionLayer(hidden_size, encoder_output_units, hidden_size, bias=False)
        else:
            self.attention = None
    ...
    def extract_features(
        ...
        # embed tokens
        x = self.word_embed(prev_output_tokens)
        x = F.dropout(x, p=self.dropout_in, training=self.training)
...
@register_model_architecture('wikilstm', 'wikilstm')
